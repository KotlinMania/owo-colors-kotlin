# Claude Code Project Instructions — owo-colors-kotlin

## Project Overview

This is **owo-colors-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`owo-colors`](https://crates.io/crates/owo-colors).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/owo-colors/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/owo-colors>
